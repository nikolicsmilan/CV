  ...
  useEffect(() => {
    ...
    const checkPerformance = () => {
      ...
      if (elapsed >= 1000) { // Check every second
        ...
        // If FPS drops below 15, consider it high load
        if (fps < 15) {
          setHighLoad(true);
...
const Icon3d = () => {
  ...
  const [allowMoreIcons, setAllowMoreIcons] = useState(true);

  const handleHighLoad = useCallback((isHighLoad) => {
    if (isHighLoad) {
      setAllowMoreIcons(false);
    } else {
      setAllowMoreIcons(true);
    }
  }, []);

  const highLoad = usePerformanceMonitor(handleHighLoad);

  useEffect(() => {
    if (isMobile && allowMoreIcons) {
      const interval = setInterval(() => {
        setVisibleIcons(prev => {
          if (prev < mobileIcons.length && allowMoreIcons) {
            return prev + 1;
          } else {
  ...
  const mobileIcons = [
    { path: glbModelreact, scale: [2, 2, 2], speed: 0.000, direction: { x:0, y: 0, z: 0 } },
    { path: glbModeljs, scale: [3, 3, 3], speed: 0.005, direction: { x: -1, y: 1, z: 1 } },
  ...
  const iconsToDisplay = isMobile ? mobileIcons.slice(0, visibleIcons) : desktopIcons;
